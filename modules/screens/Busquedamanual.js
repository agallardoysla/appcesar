...
class Busquedamanual extends Component {
  ...
  //   //Cuando term es "" trae todos las entregas pendientes
  //   // console.log("Busquedamanual-term:", term === "");
  //   //alert(term);
  ...
  busquedaManual = () => {
    ...
    if (busqueda.length) {
      ...
      if (term !== "") {
        const pendientes = busqueda.filter(
          (item) => item.matriz_codigo_factura === term
        );
        this.setState({
  ...
  renderItem = ({ item }) => (
    ...
    >
      <View style={styles.card}>
        ...
        <Text style={styles.direcciones}>
          Factura: {item.matriz_codigo_factura}
        </Text>
        <Text style={styles.direcciones}>
          Contacto: {item.cliente_telefono1}
        </Text>
      </View>
    </TouchableWithoutFeedback>
  );
  ...
  render() {
    console.log("Manual-busqueda:", this.props.busqueda.busqueda);
...
export default connect(mapStateToProps)(Busquedamanual);
